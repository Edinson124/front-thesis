<script setup>
import { bloodTypesOptions, RhFactor } from '@/enums/BloodType';
...
const columns = [
  ...
  { field: 'serologyResult', header: 'Prueba serológica' },
...
const searchUnits = async (event) => {
  loading.value = true;
  const page = event ? event.page + 1 : unitsQuarantinedStore.currentPage;
  ...
  await unitsQuarantinedStore.getUnits(params, page);
  loading.value = false;
};
...
<template>
  <div class="card">
    <!-- Sección de búsqueda -->
    <div class="mb-6">
      <h3>Búsqueda de unidades en cuarentena</h3>
    ...
    <div>
      <h2 class="text-xl font-semibold mb-4">Resultados de unidades en cuarentena</h2>

      <DataTable
        :value="unitsQuarantinedStore.units"
        :paginator="true"
        :rows="10"
        :totalRecords="unitsQuarantinedStore.totalRecords"
        :loading="loading"
